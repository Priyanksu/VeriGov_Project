...
# # --- 1. CONFIGURATION CORE & PATH REALIGNMENT ---
# app.config['SECRET_KEY'] = 'verigov_secure_cryptographic_jwt_key_2026'
# app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL')
# app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

# # Resolve absolute directory maps relative to this file's position
# BACKEND_DIR = os.path.dirname(os.path.abspath(__file__))
# UPLOAD_FOLDER = os.path.abspath(os.path.join(BACKEND_DIR, '..', 'Data', 'uploads'))
# app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER

# # Relocate ML model paths dynamically so they don't break on cloud deployment
# MODEL_PATH = os.path.abspath(os.path.join(BACKEND_DIR, 'models', 'verigov_resnet.pth'))
# NLP_MODEL_PATH = os.path.abspath(os.path.join(BACKEND_DIR, 'models', 'all-MiniLM-L6-v2'))

# print(f"[*] Flask Sentry: Mapping asset pipeline to: {UPLOAD_FOLDER}")
# print(f"[*] Model Pipeline Target: {MODEL_PATH}")

# db = SQLAlchemy(app)

# # --- 2. DATABASE MODELS LAYERS ---
# class User(db.Model):
#     __tablename__ = 'users'
...
#     # Lazy load sentence transformers to manage RAM thresholds
#     from sentence_transformers import SentenceTransformer, util
    
#     try:
#         print("🧠 Lazy Loading Local NLP Embedding Model...")
#         nlp_embedder = SentenceTransformer(NLP_MODEL_PATH)
...
#         print(f"[!] NLP Processing Exception handled gracefully: {str(e)}")
#         return "Verified", 0.50 # Fallback default to prevent blocking user journey
...
#     try:
#         # Lazy Loading PyTorch inside the endpoint invocation block
#         import torch
...
#         print("🤖 Instantiating ResNet50 Inference State Layer dynamically...")
...
#             print(f"🎉 ResNet50 weights loaded successfully.")
#         else:
#             print(f"❌ WARNING: Weights missing at local path target.")
...
#     except Exception as e:
#         # Fallback processing block if Render instances drop weights due to severe storage limits
#         print(f"[!] AI Engine Intercepted Error: {str(e)}")
#         ai_category = "potholes"  # Safe mock behavior for system runtime protection
...
#     finally:
#         # Aggressively trigger Python's garbage collector to release memory back to Render OS immediately
#         gc.collect()
...
# Fix Render's legacy postgres:// scheme for SQLAlchemy 2.0 compatibility
...
def auto_verify_text_semantics(user_description, ai_category):
    ...
    try:
        print("🧠 Lazy Loading NLP Embedding Model...")
        if os.path.exists(NLP_MODEL_PATH):
            nlp_embedder = SentenceTransformer(NLP_MODEL_PATH)
        else:
            print("🌐 Local NLP path missing on cloud. Falling back to Hugging Face online repository...")
            nlp_embedder = SentenceTransformer('sentence-transformers/all-MiniLM-L6-v2')
